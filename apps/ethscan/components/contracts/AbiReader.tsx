...
function FunctionCard({ address, fn }: { address: string; fn: AbiFunction }) {
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      ...
      >
        <span className="text-indigo-700">{fn.name}</span>
        ...
        <div className="p-4 space-y-3 text-sm">
          {fn.inputs.map((inp, i) => (
            <div key={i}>
              ...
              <input
                className="w-full border rounded px-2 py-1 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-indigo-400"
