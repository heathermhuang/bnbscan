...
export async function generateMetadata({ params }: { params: Promise<{ number: string }> }): Promise<Metadata> {
  ...
  if (isNaN(blockNumber)) return { title: 'Block Not Found — EthScan' }
  ...
  if (!block) return { title: `Block #${formatNumber(blockNumber)} — EthScan` }
  return {
    title: `Block #${formatNumber(blockNumber)} — EthScan`,
    description: `Ethereum block #${formatNumber(blockNumber)} validated by ${block.miner.slice(0, 14)}…. Contains ${block.txCount} transactions.`,
    openGraph: {
