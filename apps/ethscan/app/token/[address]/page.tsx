...
export async function generateMetadata({ params }: { params: Promise<{ address: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: `ERC-20 · ${token.holderCount.toLocaleString()} holders`,
    },
  }
}
...
export default async function TokenDetailPage({
  ...
  const totalSupplyBig = (() => {
    try { return BigInt(token.totalSupply ?? '0') } catch { return 0n }
  })()
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-left px-4 py-2 text-gray-500">Approx. Balance</th>
                ...
                return (
                  <tr key={holder.addr} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 font-mono text-xs">
                      <Link href={`/address/${holder.addr}`} className="text-blue-600 hover:underline">
                        {holder.addr}
                      </Link>
                    </td>
                    <td className="px-4 py-2">{holderAmount} {token.symbol}</td>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {riskSignals.map((s, i) => (
              <div key={i} className={`flex items-start gap-2 rounded-lg p-2 text-sm
                ${s.severity === 'danger' ? 'bg-red-50' : s.severity === 'warn' ? 'bg-amber-50' : 'bg-green-50'}`}>
      ...
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden mb-4">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            ...
            {transfers.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-gray-400">No transfers yet.</td>
              </tr>
            )}
          </tbody>
        </table>
      </div>
      <Pagination page={page} total={totalTransfers} perPage={PAGE_SIZE} baseUrl={`/token/${addr}`} />
    </div>
  )
}
