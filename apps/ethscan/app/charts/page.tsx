    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="space-y-8">
        <ChartCard title="Daily Transaction Count (Last 30 Days)">
          <LineChart data={txData} label="Transactions" formatY={(n) => n.toLocaleString()} />
        </ChartCard>

        <ChartCard title="Gas Price History — Avg Base Fee (Last 30 Days, Gwei)">
          <LineChart data={gasData} label="Gwei" formatY={(n) => `${n.toFixed(2)} Gwei`} />
        </ChartCard>

        <ChartCard title="Daily Block Count (Last 30 Days)">
          <LineChart data={blockData} label="Blocks" formatY={(n) => n.toLocaleString()} />
        </ChartCard>
      </div>
    </div>
  )
...
function LineChart({
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" style={{ minWidth: 300 }} aria-label={label}>
        {yTicks.map((t, i) => (
          <line key={i} x1={pad.left} y1={t.y} x2={width - pad.right} y2={t.y} stroke="#f0f0f0" strokeWidth="1" />
        ))}
        {yTicks.map((t, i) => (
          <text key={i} x={pad.left - 5} y={t.y + 4} textAnchor="end" fontSize="11" fill="#6b7280">
            {t.label}
          </text>
        ))}
        {xLabels.map((d, i) => {
        ...
        <polyline points={polyline} fill="none" stroke="#4F46E5" strokeWidth="2" strokeLinejoin="round" />
        {data.length <= 30 &&
          points.map((p, i) => (
