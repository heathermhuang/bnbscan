...
const PEER_URL = process.env.NEXT_PUBLIC_PEER_URL ?? 'http://localhost:3001'
        ...
        <div className={`absolute left-0 w-52 rounded-xl shadow-xl bg-white border border-gray-100 overflow-hidden z-50 ${
          ...
          {NETWORKS.map(net => {
            ...
            return (
              <div key={net.id}>
                {net.current ? (
                  <div className="flex items-center gap-3 px-3 py-2.5 bg-yellow-50">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-yellow-600 font-medium">Currently viewing</p>
                    </div>
                    <svg className="ml-auto w-4 h-4 text-yellow-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
