...
import { formatBNB, formatGwei, formatNumber, timeAgo, safeBigInt } from '@/lib/format'
...
export async function generateMetadata({ params }: { params: Promise<{ hash: string }> }): Promise<Metadata> {
  ...
  const val = formatBNB(safeBigInt(tx.value))
  return {
...
export default async function TxDetailPage({
  ...
  const [txLogs, transfers, methodName, bnbPrice, chainTip] = await Promise.all([
    fromRpc
      ? Promise.resolve([])
  ...
  const valueUsd = bnbPrice ? formatUsd(bnbValue, bnbPrice) : null
  const feeUsd = bnbPrice ? formatUsd(feeVal, bnbPrice) : null
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {fromRpc && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-4 flex items-center gap-2 text-sm text-amber-800">
          <span>⚡</span>
          <span>Fetched live from BNB Chain — this transaction predates our index.</span>
        </div>
      )}
