  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <a
          href={`https://bscscan.com/block/${block.number}`}
          target="_blank"
      ...
      {fromRpc && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-6 flex items-center gap-2 text-sm text-amber-800">
          <span>⚡</span>
          <span>Block fetched live from BNB Chain — predates our index. Click any transaction hash below to view details.</span>
        </div>
      )}
              ...
              {rpcBlock.txHashes.slice(0, 50).map(h => (
                <tr key={h} className="hover:bg-gray-50">
                  <td className="px-4 py-2 font-mono text-xs">
                    <Link href={`/tx/${h}`} className="text-yellow-600 hover:underline">{h}</Link>
                  </td>
                </tr>
              ))}
            </tbody>
